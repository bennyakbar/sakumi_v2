        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('master.fee-matrix.update', $feeMatrix) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="class_id" name="class_id"
                                    ...
                                    @foreach($classes as $class)
                                        <option value="{{ $class->id }}" {{ old('class_id', $feeMatrix->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="category_id" name="category_id"
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_id', $feeMatrix->category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="fee_type_id" name="fee_type_id"
                                    ...
                                    @foreach($feeTypes as $feeType)
                                        <option value="{{ $feeType->id }}" {{ old('fee_type_id', $feeMatrix->fee_type_id) == $feeType->id ? 'selected' : '' }}>{{ $feeType->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="amount" class="block mt-1 w-full" type="number" name="amount"
                                    :value="old('amount', $feeMatrix->amount)" min="0" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="effective_from" class="block mt-1 w-full" type="date"
                                    name="effective_from" :value="old('effective_from', $feeMatrix->effective_from->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="effective_to" class="block mt-1 w-full" type="date"
                                    name="effective_to" :value="old('effective_to', $feeMatrix->effective_to ? $feeMatrix->effective_to->format('Y-m-d') : '')" />
                            ...
                            <div class="flex items-center mt-4">
                                <label for="is_active" class="inline-flex items-center">
                                    ...
                                    <input id="is_active" type="checkbox"
                                        ...
                                        name="is_active" value="1" {{ old('is_active', $feeMatrix->is_active) ? 'checked' : '' }}>
